...
from utils.utils import communication_cost,communication_energy, communication_cost_multiple_samples,communication_energy_multiple_samples
...
class TrainerInitPop(Trainer):
    def __init__(self, model,fit_fun, num_samples=16):
    ...
    def train_step(self, data:Data, distance_matrix):
        ...
        if self.fit_fun == 'comm_energy':
            # print("**********training with comm energy fitness function******")
            comm_cost = communication_energy(data.edge_index, data.edge_attr, data.batch, distance_matrix, predicted_mappings)
        else:
...
class TrainerSR(Trainer):
    def __init__(self, model,fit_fun, num_samples=8):
        self.num_samples = num_samples
        self.fit_fun     = fit_fun
        super().__init__(model)
    def train_step(self, data:Data, distance_matrix):
        ...
        if self.fit_fun == 'comm_energy':
            # print("**********training with comm energy fitness function******")
            comm_cost, baseline = communication_energy_multiple_samples(data.edge_index, data.edge_attr, data.batch, distance_matrix, mappings, self.num_samples, calculate_baseline=True)
        else:
